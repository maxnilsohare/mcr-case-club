...
function MarqueeRow({
  ...
  const cardClass =
    "group/card relative z-0 h-[7.75rem] w-[12.75rem] shrink-0 cursor-default overflow-hidden rounded-xl border border-ink-200/65 bg-paper-200 shadow-[0_1px_0_rgba(15,23,42,0.04)] transition-[box-shadow,border-color,transform,z-index] duration-300 ease-out will-change-transform hover:z-20 hover:scale-[1.04] hover:border-ink-300/85 hover:shadow-[0_12px_32px_rgba(20,26,34,0.14)] motion-reduce:transition-none motion-reduce:hover:scale-100 sm:h-[8.5rem] sm:w-[14rem] lg:h-[9rem] lg:w-[15rem]";
...
export function HeroArchiveMarquee({
  ...
  return (
    <div
      role="region"
      aria-label="Archive photographs from competitions and training"
      className="relative overflow-hidden rounded-2xl border border-ink-200/70 bg-paper-100/80 py-3.5 shadow-[0_2px_40px_rgba(20,26,34,0.08)] sm:py-4 lg:py-5"
    >
      <div
        className="pointer-events-none absolute inset-y-0 left-0 z-10 w-10 bg-gradient-to-r from-paper-100 via-paper-100/90 to-transparent sm:w-14"
        aria-hidden
      />
      <div
        className="pointer-events-none absolute inset-y-0 right-0 z-10 w-10 bg-gradient-to-l from-paper-100 via-paper-100/90 to-transparent sm:w-14"
        aria-hidden
      />
